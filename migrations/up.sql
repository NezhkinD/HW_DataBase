...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
-- Создание схемы vehicles и таблиц
CREATE SCHEMA vehicles AUTHORIZATION dnezhkin;
...
(
    ...
    PRIMARY KEY (vin),
    FOREIGN KEY (model) REFERENCES vehicles.Vehicle (model)
);

-- Создание таблицы Motorcycle
CREATE TABLE vehicles.Motorcycle
(
    vin             VARCHAR(17)    NOT NULL,
    model           VARCHAR(100)   NOT NULL,
    engine_capacity DECIMAL(4, 2)  NOT NULL,                                                 -- объем двигателя в литрах
...
INSERT INTO vehicles.Vehicle (maker, model, type)
...
INSERT INTO vehicles.Car (vin, model, engine_capacity, horsepower, price, transmission)
...
INSERT INTO vehicles.Motorcycle (vin, model, engine_capacity, horsepower, price, type)
...
INSERT INTO vehicles.Bicycle (serial_number, model, gear_count, price, type)
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
-- Создание схемы racing
CREATE SCHEMA racing AUTHORIZATION dnezhkin;

-- Создание таблицы Classes
CREATE TABLE racing.Classes
(
...
(
    ...
    year  INT          NOT NULL,
    PRIMARY KEY (name),
    FOREIGN KEY (class) REFERENCES racing.Classes (class)
);
...
(
    ...
    FOREIGN KEY (car) REFERENCES racing.Cars (name),
    FOREIGN KEY (race) REFERENCES racing.Races (name)
);

-- Вставка данных в таблицу Classes
INSERT INTO racing.Classes (class, type, country, numDoors, engineSize, weight)
...
INSERT INTO racing.Cars (name, class, year)
...
INSERT INTO racing.Races (name, date)
...
INSERT INTO racing.Results (car, race, position)
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
CREATE SCHEMA hb AUTHORIZATION dnezhkin;

-- Создание таблицы Hotel
CREATE TABLE hb.Hotel
(
...
(
    ...
    FOREIGN KEY (ID_hotel) REFERENCES hb.Hotel (ID_hotel)
);

-- Создание таблицы Customer
CREATE TABLE hb.Customer
(
...
(
    ...
    FOREIGN KEY (ID_room) REFERENCES hb.Room (ID_room),
    FOREIGN KEY (ID_customer) REFERENCES hb.Customer (ID_customer)
);

-- Вставка данных в таблицу Hotel
INSERT INTO hb.Hotel (ID_hotel, name, location)
...
INSERT INTO hb.Room (ID_room, ID_hotel, room_type, price, capacity)
...
INSERT INTO hb.Customer (ID_customer, name, email, phone)
...
INSERT INTO hb.Booking (ID_booking, ID_room, ID_customer, check_in_date, check_out_date)
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
CREATE SCHEMA org AUTHORIZATION dnezhkin;
...
(
    ...
    DepartmentID INT,
    RoleID       INT,
    FOREIGN KEY (ManagerID) REFERENCES org.Employees (EmployeeID) ON DELETE SET NULL,       -- Устанавливаем поведение при удалении
    FOREIGN KEY (DepartmentID) REFERENCES org.Departments (DepartmentID) ON DELETE CASCADE, -- Устанавливаем поведение при удалении
    FOREIGN KEY (RoleID) REFERENCES org.Roles (RoleID) ON DELETE SET NULL                   -- Устанавливаем поведение при удалении
);
...
(
    ...
    AssignedTo INT,
    ProjectID  INT,
    FOREIGN KEY (AssignedTo) REFERENCES org.Employees (EmployeeID) ON DELETE SET NULL, -- Устанавливаем поведение при удалении
    FOREIGN KEY (ProjectID) REFERENCES org.Projects (ProjectID) ON DELETE CASCADE      -- Устанавливаем поведение при удалении
);
...
-- Добавление ролей
INSERT INTO org.Roles (RoleID, RoleName)
VALUES (1, 'Менеджер'),
       (2, 'Директор'),
...
INSERT INTO org.Employees (EmployeeID, Name, Position, ManagerID, DepartmentID, RoleID)
...
-- Добавление проектов
INSERT INTO org.Projects (ProjectID, ProjectName, StartDate, EndDate, DepartmentID)
...
-- Добавление задач
INSERT INTO org.Tasks (TaskID, TaskName, AssignedTo, ProjectID)
